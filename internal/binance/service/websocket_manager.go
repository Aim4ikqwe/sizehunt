...
func (m *WebSocketManager) LoadActiveSignals() error {
	...
	for _, userID := range userIDs {
		...
		// Для каждого сигнала создаем watcher и добавляем сигнал
		for _, signalDB := range signals {
			watcher, err := m.GetOrCreateWatcherForUser(
				userID,
				signalDB.Symbol,
				signalDB.WatchMarket,
				signalDB.AutoClose,
			)
			if err != nil {
			...
			log.Printf("WebSocketManager: Loaded signal %d for user %d, symbol %s", signalDB.ID, userID, signalDB.Symbol)
		}
	}
