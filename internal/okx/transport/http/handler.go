...
func (h *Handler) DeleteSignal(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("OKXHandler: ERROR: Failed to get signal %d: %v", signalID, err)
		http.Error(w, "signal not found", http.StatusNotFound)
		return
	}

	if signal.UserID != userID {
		log.Printf("OKXHandler: ERROR: User %d attempting to delete signal %d that belongs to user %d", userID, signalID, signal.UserID)
