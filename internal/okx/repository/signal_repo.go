...
type SignalRepository interface {
	...
	GetActiveByUserAndInstID(ctx context.Context, userID int64, instID string) ([]*SignalDB, error)
...
// NewPostgresSignalRepo создает новый репозиторий сигналов OKX
...
// GetActiveByUserAndInstID получает все активные сигналы для пользователя и инструмента
func (r *PostgresSignalRepo) GetActiveByUserAndInstID(ctx context.Context, userID int64, instID string) ([]*SignalDB, error) {
	query := `
		SELECT id, user_id, inst_id, inst_type, target_price, min_quantity, trigger_on_cancel, 
		       trigger_on_eat, eat_percentage, original_qty, last_qty, auto_close, 
		       original_side, created_at, updated_at, is_active
	...
	`
	rows, err := r.DB.QueryContext(ctx, query, userID, instID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *PostgresSignalRepo) Delete(ctx context.Context, signalID int64) error {
	_, err := r.DB.ExecContext(ctx, "DELETE FROM okx_signals WHERE id = $1", signalID)
	return err
}
...
func (r *PostgresSignalRepo) Deactivate(ctx context.Context, signalID int64) error {
	_, err := r.DB.ExecContext(ctx,
		`UPDATE okx_signals SET is_active = false, updated_at = NOW() WHERE id = $1`,
		signalID)
	return err
}
